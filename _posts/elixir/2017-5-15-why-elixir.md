...
最后一个难题是得有个工具帮助我们管理项目，Elixir自带的mix正是这样的工具。它又一次让我们感受到简洁，当你创建一个新项目，仅仅会创建7个文件（包括.gitignore和README.md），而这却是创建OPT应用的全部。这是一个复杂事情简单化的极好示例。

Mix支持各种各样的任何，比如依赖管理，而且还具有扩展性，你可以根据需要定义你自己的任务。


## 语法变化

Elixir带给我们的好处不仅仅局限于此，还有许多语法上的改进，比如支持变量的重新绑定，可选的括号，隐含的语句结尾，nullability以及短路操作符。
不可否认的是，可选的括号确实会带来一定的歧义，比如这个示例：

```elixir
abs -1 + 5  # same as abs(-1 + 5)
```

由于我总是使用括号（除非是宏和无参数函数），因此我想不起在这方面碰到什么问题。通常来说Elixir的这些语法改进我都是挺喜欢的，可以写没有else子句的if很让人喜欢，不需要判断语句用什么结尾也很让人高兴。

可选的括号对于支持使用宏实现DSL很有帮助，它让代码更加精炼，否则我们就必须要使用括号来调用宏。

```elixir
defrecord User, name: nil, age: 0       # 省略括号

defrecord(User, [name: nil, age: 0])    # 使用括号
```

当然这些改进算不上极其重要的，它们起到极好的润色作用，如果仅仅是这些特性，那我还是会选择Erlang。


## 总结

这篇文章说了很多，但Elixir的魔法还远远不止这些。虽然语言的偏好有着某种主观性，但Elixir确实在Erlang基础上有不少改进。在Erlang大约三年经验，以及一年的Elixir经验，我发现使用Elixir更加愉快，产生的代码更加紧凑，我更加专注于正在解决的问题，而不是和代码较劲。

Elixir和EVM都提供了抽象机制，帮助我们更加轻松地处理问题。这就是为什么我把钱放到Elixir/EVM的组合上，把它们作为构建服务端系统的首选工具。 YMMV当然是 <--- 这是什么东东
